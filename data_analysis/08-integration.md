...
### Label transfer from published dataset

The number of published single cell RNASeq dataset is increasing significantly as the technology matures. One application that takes advantage of the published dataset is to annotate a new single cell RNASeq dataset by label transfer. The procedure of transfer the cell type labels from the published dataset involves some similar processes as the integration of datasets. Both procedures are based on a fundamental concept of finding mutual nearest neighbors (MNN) to define anchors across different datasets.


``` r
## prepare an example
## read in the object that has celltype labels
experiment.celltype <- readRDS("scRNA_workshop-05.rds")

## read in the object that includes all samples, the same as in the above section
...
## UMAP for visualization later
  sce = RunPCA(sce)
  sce = RunUMAP(sce, dims = 1:50, return.model = TRUE)
})

## use A001-C-007 as the reference, add celltype information from Part 5 rds (scRNA_workshop-05.rds)
experiment.aggregate <- readRDS("scRNA_workshop-05.rds")
experiment.split[["A001-C-007"]]@meta.data$celltype <- experiment.aggregate$subcluster_ScType_2tissue[match(colnames(experiment.split[["A001-C-007"]]), colnames(experiment.aggregate))]

## find transfer anchors
anchors <- FindTransferAnchors(reference = experiment.split[["A001-C-007"]],
	query = experiment.split[["A001-C-104"]],
	reduction = "pcaproject",
	reference.reduction = "pca")
celltype.predicted <- TransferData(anchorset = anchors,
                                   refdata = experiment.split[["A001-C-007"]]$celltype)

## assign predicted celltype to cells
experiment.split[["A001-C-104"]] <- AddMetaData(experiment.split[["A001-C-104"]],
                                                metadata = celltype.predicted)

## project the query onto the reference UMAP structure
experiment.split[["A001-C-104"]] <- MapQuery(anchorset = anchors,
	reference = experiment.split[["A001-C-007"]],
	query = experiment.split[["A001-C-104"]],
...
``` r
DimPlot(experiment.split[["A001-C-104"]],
        group.by = "predicted.id",
        reduction = "ref.umap",
        label = TRUE)
```

![](08-integration_files/figure-html/unnamed-chunk-3-1.png)<!-- -->
...
![](08-integration_files/figure-html/unnamed-chunk-3-2.png)<!-- -->
